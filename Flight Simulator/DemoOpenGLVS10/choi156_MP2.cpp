...
void init(void)
{
	...
	glClearColor(0.282, 0.239, 0.545, 0.0);	// sky
	glEnable(GL_DEPTH_TEST);

	glLoadIdentity();
	gluLookAt(-0.25, 2,2,2,0,0,0,0,1);
...
void display(void)
{
	...
	glTranslatef(0, 0, velocity);//Grant the plane the initial velocity
	glRotatef(roll, 0.0, 0.0, 1.0);//rotate on -z axis
	glRotatef(pitch, 1.0, 0.0, 0.0);//rotate on x axis
	glMultMatrixf(curr);//multiply transformation matrix
...
//function that gets arrow inputs for pitch and roll.
void arrow (int key, int x, int y) {
	switch (key) {
	case GLUT_KEY_UP://pitch up
		pitch-=.05;
		if(pitch <= -0.5)//statement to limit maximum pitch speed.
		{
			pitch = -0.5;
		}
		break;
	...
	case GLUT_KEY_LEFT://roll left
		roll-=.05;
		if(roll <= -0.7)//statement to limit maximum roll speed.
		{
			roll = -0.7;
		}
		break;
...
int main(int argc, char** argv)
{
	...
	glutSpecialFunc(arrow);//arrow key input function
